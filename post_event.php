...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["post_event"])) {
        ...
        // Check file size
        if ($_FILES["event_photo"]["size"] > 5000000) {
        ...
        if ($uploadOk == 0) {
            $message = "Sorry, your file should not exceed 5MB.";
        // if everything is ok, try to upload file
        } else {
            if (move_uploaded_file($_FILES["event_photo"]["tmp_name"], $target_file)) {
                // Insert data into MySQL database
                $sql = "INSERT INTO events (photo, description, title, event_date, event_time, venue) 
                        VALUES ('$target_file', '$description', '$title', '$event_date', '$event_time', '$venue')";
        ...
        <div class="form-container">
            ...
            <form action="post_event.php" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group d-flex align-items-end">
                    <button type="button" class="btn btn-primary btn-custom register-btn" id="register-button">Register</button>
                </div>
